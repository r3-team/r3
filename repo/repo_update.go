		...
		if !exists {
			if _, err := tx.Exec(db.Ctx, `
				INSERT INTO instance.repo_module (
					module_id_wofk, name, author, in_store, release_build,
					release_build_app, release_date, file
				)
				VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
			`, sm.ModuleId, sm.Name, sm.Author, sm.InStore, sm.ReleaseBuild,
				sm.ReleaseBuildApp, sm.ReleaseDate, sm.FileId); err != nil {
			...
			if sm.ReleaseBuild == 0 {
				if _, err := tx.Exec(db.Ctx, `
					...
					SET name = $1, author = $2, in_store = $3
					WHERE module_id_wofk = $4
				`, sm.Name, sm.Author, sm.InStore, sm.ModuleId); err != nil {
					return err
				}
			} else {
				if _, err := tx.Exec(db.Ctx, `
					UPDATE instance.repo_module
					SET name = $1, author = $2, in_store = $3,
						release_build = $4, release_build_app = $5,
						release_date = $6, file = $7
					WHERE module_id_wofk = $8
				`, sm.Name, sm.Author, sm.InStore, sm.ReleaseBuild,
					sm.ReleaseBuildApp, sm.ReleaseDate, sm.FileId,
					sm.ModuleId); err != nil {
