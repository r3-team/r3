...
func getActions(formId uuid.UUID) ([]types.FormAction, error) {
	...
	rows, err := db.Pool.Query(db.Ctx, `
		SELECT id, js_function_id, icon_id, color
	...
	for rows.Next() {
		...
		if err := rows.Scan(&a.Id, &a.JsFunctionId, &a.IconId, &a.Color); err != nil {
			return actions, err
		}
...
func setAction_tx(tx pgx.Tx, formId uuid.UUID, a types.FormAction, position int) (uuid.UUID, error) {
	...
	if known {
		if _, err := tx.Exec(db.Ctx, `
			...
			SET js_function_id = $1, icon_id = $2, position = $3, color = $4
			WHERE id = $5
		`, a.JsFunctionId, a.IconId, position, a.Color, a.Id); err != nil {
			return a.Id, err
		}
	} else {
		if _, err := tx.Exec(db.Ctx, `
			INSERT INTO app.form_action (
				id, form_id, js_function_id, icon_id, position, color
			)
			VALUES ($1,$2,$3,$4,$5,$6)
		`, a.Id, formId, a.JsFunctionId, a.IconId, position, a.Color); err != nil {
			return a.Id, err
		}
	}
