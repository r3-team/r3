...
func Set_tx(tx pgx.Tx, loginId int64, languageCode string, dateFormat string,
	...
	menuColored bool, pattern pgtype.Varchar) error {
...
func SetDefaults_tx(tx pgx.Tx, id int64, languageCode string) error {
	_, err := tx.Exec(db.Ctx, `
		INSERT INTO instance.login_setting (login_id, language_code, date_format,
			...
			mobile_scroll_form, warn_unsaved, menu_colored, pattern)
		VALUES ($1,$2,'Y-m-d',true,100,false,'keep',2000,true,3,false,true,0,true,true,false,'bubbles')
	`, id, languageCode)
	return err
}
