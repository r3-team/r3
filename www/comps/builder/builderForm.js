...
import { getIndexAttributeId, isAttributeRelationship, isAttributeRelationshipN1 } from '../shared/attribute.js';
import { getDependentRelations, getFormEntityMapRef } from '../shared/builder.js';
import {
	getTemplateFieldButton, getTemplateFieldCalendar, getTemplateFieldChart, getTemplateFieldContainer,
	getTemplateFieldData, getTemplateFieldGantt, getTemplateFieldHeader, getTemplateFieldKanban,
	getTemplateFieldList, getTemplateFieldMap, getTemplateFieldTabs, getTemplateFieldVariable, getTemplateQuery
} from '../shared/builderTemplate.js';
import { dialogDeleteAsk } from '../shared/dialog.js';
import { getDataFields, getFormRoute } from '../shared/form.js';
...
export default {
	...
	computed: {
		...
		fieldsTemplate: {
			get() {
				...
				fields.push(this.getTemplateFieldContainer());
				fields.push(this.getTemplateFieldTabs());
				fields.push(this.getTemplateFieldList());
				fields.push(this.getTemplateFieldCalendar());
	...
	methods: {
		...
		addLayoutColumns(count) {
			const parent = this.getTemplateFieldContainer();
			parent.direction = 'row';
			parent.wrap = true;

			for (; count > 0; count--) {
				const child = this.getTemplateFieldContainer();
				child.basis = 300;
		...
		createFieldsForRelation(relation, index) {
			...
			for (const atr of relation.attributes) {
			...
			const remove = fields => {
				for (let i = 0, j = fields.length; i < j; i++) {
					...
					switch (f.content) {
						...
						case 'tabs': for (const t of f.tabs) { remove(t.fields); } break;
					}
				}
			};
		...
		resetRouteParams() {
			const params = { fieldIdShow: { parse: 'string', value: null } };
