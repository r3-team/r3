...
import { getDependentDocs, getItemTitle, getItemTitleRelation } from '../shared/builder.js';
	...
	computed: {
		...
		doc: {
			...
			set(v) {
				...
				const o = this.active ? JSON.parse(JSON.stringify(this.modelValue)) : this.getTemplateOpenDoc();
				o.relationIndexOpen = parseInt(parts[0], 10);
				o.docIdOpen = parts[1];
		...
		fieldIdAddTo: {
			...
			set(v) {
				const o = JSON.parse(JSON.stringify(this.modelValue));
