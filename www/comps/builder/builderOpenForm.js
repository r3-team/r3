...
import { getDependentModules, getDependentRelations, getItemTitleRelation } from '../shared/builder.js';
...
export default {
	...
	props: {
		...
		joinsIndexMapField: { type: Object, required: false, default: () => { return {}; } },
	...
	computed: {
		...
		form: {
			...
			set(v) {
				const o = JSON.parse(JSON.stringify(this.openForm));
				...
				const p = v.split('_');
				if (p.length === 2) {
					o.relationIndexOpen = parseInt(p[0], 10);
					o.formIdOpen = p[1];
				}
				this.openForm = o;
			}
		...
		targetAttributes: s => {
			...
			for (const join of form.query.joins) {
				...
				for (const atr of s.relationIdMap[relationIdRecord].attributes) {
