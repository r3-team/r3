...
import { pemImport, pemImportPrivateEnc } from './shared/crypto.js';
import { consoleError, genericError, genericErrorWithFallback, resolveErrCode } from './shared/error.js';
...
export default {
	...
	computed: {
		...
		styles: s => {
			...
			const styles = [`font-size:${s.settings.fontSize}%`];
		...
		moduleEntries: s => {
			...
			const parseModule = (module, topLevel) => {
				...
				const formIdStart = s.getStartFormId(module, s.access);
				const accessible = formIdStart !== null;
				...
				if (typeof idMapChildren[module.id] !== 'undefined') {
					for (let i = 0, j = idMapChildren[module.id].length; i < j; i++) {
						const m = parseModule(idMapChildren[module.id][i], false);
	...
	methods: {
		...
		sessionExpireCheck() {
			...
			this.logoutInSec = this.loginSessionExpires - 1800 < now ? this.loginSessionExpires - now : 0;
		},
		...
		// hotkeys
		handleKeydown(e) {
			for (const k of this.keyDownHandlers) {
