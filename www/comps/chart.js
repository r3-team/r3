...
import { getQueryExpressions, getRelationsJoined } from './shared/query.js';
import { getUnixFormat, getUnixShifted, getUtcTimeStringFromUnix } from './shared/time.js';
	...
	computed: {
		dateTimeColumnIndexes: s => {
			...
			for (let i = 0, j = s.columns.length; i < j; i++) {
				const atr = s.attributeIdMap[s.columns[i].attributeId];
