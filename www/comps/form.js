...
import {
	getAttributeFileVersionHref, getAttributeValueFromString, getDetailsFromIndexAttributeId,
	getGetterFromAttributeValues, getIndexAttributeId, getIndexAttributeIdByField,
	isAttributeRelationship, isAttributeRelationshipN1,
...
import {
	...
	getRandomString, pemImport, rsaDecrypt, rsaEncrypt
} from './shared/crypto.js';
import { dialogCloseAsk } from './shared/dialog.js';
import { consoleError } from './shared/error.js';
import { getFieldOverwriteDefault, getFieldProcessedDefault } from './shared/field.js';
import {
	getDataFieldMap, getFormPopUpConfig, getFormRoute,
	getFormStateIdMap, getResolvedPlaceholders, getRowsDecrypted
} from './shared/form.js';
import { filterIsCorrect, filterOperatorIsSingleValue, openLink, openLinkNoCache } from './shared/generic.js';
...
export default {
	name: 'my-form',
	components: { MyArticles, MyField, MyFormActions, MyFormLog },
	...
	computed: {
		...
		valuesDef: s => {
			...
			const parseFields = fields => {
				for (const f of fields) {
					...
					let def = null;
					const attribute = s.attributeIdMap[f.attributeId];
					const indexAttributeId = s.getIndexAttributeIdByField(f, false);
					const isRelationship = s.isAttributeRelationship(attribute.content);
					const isRelationshipN1 = s.isAttributeRelationshipN1(attribute.content);
					const isRelMulti = isRelationship && f.attributeIdNm !== null || (f.outsideIn && isRelationshipN1);
		...
		fieldIdMapProcessed: s => {
			const out = s.getFieldProcessedDefault();
			...
			const parseFields = fields => {
				for (const f of fields) {
					...
					const isFieldWithQuery = f.query !== undefined && f.query !== null;
					const isFieldMapWithQuery = f.content === 'map' && f.layersData.length !== 0;

					if (!isFieldWithQuery && !isFieldMapWithQuery)
						continue;

					const collectionIdMap = s.$root.getOrFallback(s.fieldIdMapOptions[f.id], 'collectionIdMapIndexes', {});

					if (isFieldWithQuery) {
						const choices = JSON.parse(JSON.stringify(f.query.choices));
					...
					if (isFieldMapWithQuery) {
						out.layerFilters[f.id] = {};
						for (const l of f.layersData) {
							out.layerFilters[f.id][l.id] = s.getQueryFiltersProcessed(
								l.query.filters, s.joinsIndexMap, null, null, s.fieldIdMapData, s.fieldIdsChanged, s.fieldIdsInvalid, s.values,
								s.mayCreate, s.mayDelete, s.mayUpdate, collectionIdMap, s.variableIdMapLocal
							);
						}
					}
	...
	methods: {
		...
		valueSetByRows: async function (rows, expressions) {
			...
			for (const index in row.indexRecordEncKeys) {
		...
		triggerEvent(event, before) {
			for (const f of this.form.functions) {
		...
		getFromSubJoin(join, recordId) {
			const joinIndexes = [join.index]; // all join indexes to collect (start with initial join)
			const joins = [];           // all collected joins
			let joinAdded = true;
			...
			while (joinAdded) {
				...
				for (const r of this.relationsJoined) {
			...
			const filters = this.getQueryFiltersProcessed(
		...
		set: async function (saveAndNew, saveAndClose) {
			...
			const relations = {};
			const addRelationByIndex = async index => {
			...
			for (const k in this.values) {
				const d = this.getDetailsFromIndexAttributeId(k);
				const j = this.joinsIndexMap[d.index];
				const isNew = j.recordId === 0;
			...
			const requests = [ws.prepare('data', 'set', relations)];
			...
			for (let i = 0, j = this.loginIdsEncryptForOutside.length; i < j; i++) {
				try {
					...
					const [resDataKeys, resLoginKeys] = await Promise.all([
					...
					const recordIdMapKeyStr = {}; // data key, by record ID, unencrypted
					const recordIdMapKeysEnc = {}; // data keys, by record ID, encrypted with public keys
					...
					for (const recordId in recordIdMapKeysEnc) {
		...
		setBulkUpdate() {
			...
			const attributes = [];
			for (const fieldId of this.fieldIdsTouched) {
