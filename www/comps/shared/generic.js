...
export function colorDarken(color, percent) {
	return tinycolor(color).darken(percent);
};

export function colorApplyAlpha(color, alpha) {
...
export function checkDataOptions(requested, options) {
	...
	switch (requested) {
		case 1: return [1, 3, 5, 7].includes(options); // DELETE
		case 2: return [2, 3, 6, 7].includes(options); // UPDATE
		case 4: return [4, 5, 6, 7].includes(options); // CREATE
	}
	return false;
};
...
export function filterIsCorrect(operator, value0, value1) {
	switch (operator) {
		case 'IS NULL': return value0 === null;
		case 'IS NOT NULL': return value0 !== null;
		case '=': return value0 == value1;
		case '<>': return value0 != value1;
		case '<': return value0 < value1;
		case '>': return value0 > value1;
		case '<=': return value0 <= value1;
		case '>=': return value0 >= value1;
		case 'LIKE': return new RegExp(value0).test(value1);
		case 'ILIKE': return new RegExp(value0, 'i').test(value1);
	}
