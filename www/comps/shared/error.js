...
export function resolveErrCode(message) {
	...
	const matches = message.match(/^{ERR_([A-Z]{3})_(\d{3})}/);
	...
	const cap = MyStore.getters.captions.error[errContext][errNumber];

	// handle cases with error context data
	if (errContext === "APP") {
		switch (errNumber) {
			case '015':
				return cap.replace("{NAME}", data.name).replace('{MAIL}', data.mail);
		}
	}
	if (errContext === 'CSV') {
		switch (errNumber) {
	...
	if (errContext === 'SEC') {
		switch (errNumber) {
			case '006': return cap.replace('{NAMES}', data.names);
		}
	}
	if (errContext === 'TRF') {
		switch (errNumber) {
			case '001': return cap.replace('{BUILD_LOCAL}', data.buildLocal).replace('{BUILD_REPO}', data.buildRepo);
		}
	}
	return cap;
}
